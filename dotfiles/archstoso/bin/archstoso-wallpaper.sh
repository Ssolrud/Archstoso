...
# Load Settings
if [ -f ~/.config/archstoso/settings/wallpaper-folder ]; then
    SEARCH_PATH=$(cat ~/.config/archstoso/settings/wallpaper-folder)
fi
...
# --- 3. UI Selection with fzf ---
if [ -z "$LIST" ]; then
    # Final check for the directory existence itself
    if [ ! -d "$SEARCH_PATH" ]; then
        echo "Error: Directory '$SEARCH_PATH' not found. Please create it."
        echo ":: You can define your own Wallpaper folder in ~/.config/archstoso/settings/wallpaper-folder"
    else
