...
sec=$(cat ~/.config/archstoso/settings/wallpaper-automation.sh)
...
if [ ! -f $archstoso_cache_folder/wallpaper-automation ]; then
    touch $archstoso_cache_folder/wallpaper-automation
    ...
    rm $archstoso_cache_folder/wallpaper-automation
    notify_user \
        --a "${APP_NAME}" \
        --i "${NOTIFICATION_ICON}" \
