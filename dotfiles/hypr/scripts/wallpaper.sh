...
# Source library.sh
source $HOME/.config/archstoso/library.sh
...
if [ -f ~/.config/archstoso/settings/wallpaper_cache ]; then
    use_cache=1
...
if [ ! -d $archstoso_cache_folder ]; then
    mkdir -p $archstoso_cache_folder
fi
...
generatedversions="$archstoso_cache_folder/wallpaper-generated"
if [ ! -d $generatedversions ]; then
    mkdir -p $generatedversions
fi

# Will be set when waypaper is running
waypaperrunning=$archstoso_cache_folder/waypaper-running
if [ -f $waypaperrunning ]; then
    rm $waypaperrunning
    exit
fi

cachefile="$archstoso_cache_folder/current_wallpaper"
blurredwallpaper="$archstoso_cache_folder/blurred_wallpaper.png"
squarewallpaper="$archstoso_cache_folder/square_wallpaper.png"
rasifile="$archstoso_cache_folder/current_wallpaper.rasi"
blurfile="$HOME/.config/archstoso/settings/blur.sh"
defaultwallpaper="$HOME/.config/archstoso/wallpapers/default.jpg"
wallpapereffect="$HOME/.config/archstoso/settings/wallpaper-effect.sh"
blur="50x30"
blur=$(cat $blurfile)
